...
export default function Header({ activePage, onNavigate, hasAnalysis }: HeaderProps) {
  return (
    <header className="bg-white border-b border-gray-200 sticky top-0 z-30 w-full shadow-xs">
      <div className="max-w-[1400px] mx-auto px-6 sm:px-8 lg:px-12 py-3.5 flex items-center justify-between">
        {/* Brand */}
        <div
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-base sm:text-lg font-bold text-[#1e3a5f] tracking-tight leading-none">
              NauSaarthi
            </span>
            <span className="text-xs text-gray-500 hidden sm:inline leading-tight mt-1 font-medium">
        ...
        <nav className="flex items-center gap-2">
          <button
            ...
            className={`px-4 py-2 text-xs sm:text-sm font-medium rounded-lg transition-colors cursor-pointer ${
              activePage === 'overview'
                ? 'bg-[#eef2ff] text-[#1e3a5f] font-semibold shadow-2xs'
          ...
          <button
            ...
            className={`px-4 py-2 text-xs sm:text-sm font-medium rounded-lg transition-colors ${
              activePage === 'analysis'
                ? 'bg-[#eef2ff] text-[#1e3a5f] font-semibold shadow-2xs cursor-pointer'
                : hasAnalysis
